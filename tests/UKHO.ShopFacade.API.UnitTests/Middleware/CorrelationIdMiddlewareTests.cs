...
namespace UKHO.ShopFacade.API.UnitTests.Middleware
{
    ...
    public class CorrelationIdMiddlewareTests
    {
        ...
        {
            var correlationId = Guid.NewGuid().ToString();
            _fakeHttpContext.Request.Headers[ApiHeaderKeys.XCorrelationIdHeaderKey] = correlationId;

            await _middleware.InvokeAsync(_fakeHttpContext);

            A.CallTo(() => _fakeHttpContext.Response.Headers.ContainsKey(ApiHeaderKeys.XCorrelationIdHeaderKey)).Returns(true);
            A.CallTo(() => _fakeHttpContext.Response.Headers[ApiHeaderKeys.XCorrelationIdHeaderKey]).Returns(correlationId);
